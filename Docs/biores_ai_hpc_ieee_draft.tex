...
\begin{equation}
\begin{split}
\text{Safe and Verified Recovery}
&= \text{Functional Restoration} \\
&\quad \land \text{Post-Recovery Verification}.
\end{split}
\label{eq:safe-verified-recovery}
\end{equation}
...
&\quad + T_{\mathrm{rest}}
 + T_{\mathrm{ver}}.
\end{aligned}
\label{eq:safe-recovery-time}
\end{equation}

Here, $T_{\mathrm{det}}$ is the detection time,
$T_{\mathrm{iso}}$ is the isolation and containment time,
$T_{\mathrm{reconf}}$ is the reconfiguration time,
$T_{\mathrm{rest}}$ is the restoration time, and
$T_{\mathrm{ver}}$ is the post-recovery verification time.
...
where $f_{\mathrm{data}}$, $f_{\mathrm{train}}$,
$f_{\mathrm{ckpt}}$, $f_{\mathrm{inf}}$,
$f_{\mathrm{val}}$, and $f_{\mathrm{orch}}$ denote data ingestion and
preparation, training, checkpointing, inference, validation, and workflow
...
\begin{aligned}
\text{Nominal}
&\rightarrow \text{Detected}
\rightarrow \text{Isolated} \\
&\rightarrow \text{Degraded}
\rightarrow \text{Restored}
\rightarrow \text{Verified}.
\end{aligned}
\label{eq:recovery-states}
\end{equation}
...
\begin{aligned}
\max \quad
& \left(FC, C_R, \text{efficiency}\right) \\
\text{subject to} \quad
& T_{\mathrm{SR}} \leq T_{\mathrm{SR}}^{\max}, \\
& O_R \leq O_R^{\max}, \\
& \text{safety constraints are satisfied}.
\end{aligned}
\label{eq:multi-objective-design}
\end{equation}
